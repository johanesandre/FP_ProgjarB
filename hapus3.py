...
def main():
  ...
  print "qsize : %s" % clientQueue.qsize()
  while 1:
    try:
      print 'waiting connection'
      conn, addr = sock.accept()
      #clientQueue.put(conn, False)
      #conn = clientQueue.get()
      worker = connectToServer(conn)
      worker.setDaemon(True)
      worker.start()
      #print "qsize : %s" % clientQueue.qsize()
      #clientQueue.put(conn, True)
      print 'connected from conn %s ip %s' %(conn, addr)
      print type(conn)
      print type(addr)
      #data = clientQueue.get(True)
